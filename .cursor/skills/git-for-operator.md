...
| Doc | Use when |
|-----|----------|
| [CONTRIBUTING.md](../../CONTRIBUTING.md) | Commit scopes (`feat(unit-NN/slug): …`), `shared/v1.2.1` baseline, `chore(shared):`, **infra tags** (`infra/onboarding-skills/v*`) |
...
| Registry / CONTRIBUTING tag table only (no skills) | `docs: update shared v1.2.1 baseline in CONTRIBUTING` | (usually no tag) |
| General `shared/` behaviour (all unis) | `chore(shared): describe the behaviour change in one line` | `shared: same one-line summary as commit` → next patch tag (e.g. `shared/v1.2.2`) per CONTRIBUTING |
...
| Pin `shared/` to baseline only | `chore(shared): restore shared to shared/v1.2.1 baseline` | (no new shared tag — matches existing `shared/v1.2.1`) |
