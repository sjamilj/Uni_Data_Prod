...
from course_markdown_cleanup import cleanup_course_markdown, cleanup_uni_markdown, uni_source_url
...
class CourseMarkdownCleanupBridge:
    ...
    def cleanup_uni(self, markdown: str, **kwargs: object) -> str:
        return cleanup_uni_markdown(markdown, code_dir=self.code_dir, **kwargs)

    def uni_source_url(self, raw_source: str, **kwargs: object) -> str | None:
        return uni_source_url(raw_source, code_dir=self.code_dir, **kwargs)
...
class UniversityNameResolver:
    ...
    def resolve(work_dir: Path) -> str:
        ...
        if not env_path.exists():
            raise ValueError(f"Missing {ENV_FILE}; set UNIVERSITY_NAME= in .env")
        env = load_env_file(env_path)
        name = (env.get("UNIVERSITY_NAME") or "").strip()
        if not name:
            raise ValueError(f"UNIVERSITY_NAME is required in {env_path}")
        return name
...
class UniReqDirResolver:
    ...
    def resolve(work_dir: Path) -> Path | None:
        ...
        for candidate in (work_dir.parent / UNI_REQ_DIR, work_dir / UNI_REQ_DIR):
            if candidate.is_dir():
                return candidate
        return None

    @staticmethod
    def source_html_rel(html_path: Path, work_dir: Path) -> str:
        html_path = html_path.resolve()
        work_dir = work_dir.resolve()
        for base in (work_dir.parent, work_dir):
            try:
...
class CleanWarningsWriter:
    ...
    FIELDNAMES = ("source_html", "source_url", "warning_type", "detail")

    @classmethod
    def write(cls, output_dir: Path, warnings: list[CleanWarning]) -> Path:
        path = output_dir / CLEAN_WARNINGS_CSV
        with path.open("w", encoding="utf-8", newline="") as handle:
            writer = csv.DictWriter(handle, fieldnames=cls.FIELDNAMES)
            writer.writeheader()
...
class CourseMarkdownBuilder:
    ...
    def from_config(
        ...
        *,
        ...
        soup = BeautifulSoup(html, "html.parser")
        engine = get_course_html_engine(clean_config.engine, code_dir)
        sections: list[str] = []

        title = engine.course_title_from_soup(soup, clean_config)
        if title:
            sections.append(f"# {title}")

        blocks_produced = 0
        for env_heading, selector in clean_config.blocks:
            node, resolved_selector = engine.find_block(soup, env_heading, selector)
            if not node:
                detail = selector
                print(f"  Warning: clean block not found: {detail}")
            ...
            engine.strip_within_block(block_root, clean_config.strip_within)

            heading = env_heading or engine.derive_heading_from_block(block_root)
            ...
            block = engine.append_block_extras(block_root, selector, block)
            sections.append(block)
            blocks_produced += 1

        if blocks_produced == 0 and not title:
            print("  Warning: no clean blocks produced content")
            if warnings is not None:
                warnings.append(
                    CleanWarning(
                        source_html=source_html,
...
class CourseEntryCatalog:
    ...
    def read_entries(self) -> list[tuple[str, Path]]:
        ...
        if map_path.exists():
            with map_path.open(newline="", encoding="utf-8") as handle:
                for row in csv.DictReader(handle):
                    ...
                    html_rel = (row.get("html_path") or row.get("html_file") or "").strip()
                    if not html_rel:
                        continue
                    html_path = self.output_dir / Path(html_rel)
                    if html_path.exists():
                        entries.append((course_url, html_path))
        ...
        for html_path in sorted(pages_dir.glob("*.html")):
            ...
            course_url = HtmlSourceExtractor.extract_source_url(raw_html)
            entries.append((course_url, html_path))
        return entries
...
class ManifestWriter:
    ...
    @staticmethod
    def _merge_course_lists(existing: list[dict], incoming: list[dict]) -> list[dict]:
        merged: dict[tuple[str, str], dict] = {}
        for item in existing:
            key = (
                normalize_url(str(item.get("course_url") or "")),
                str(item.get("study_level") or ""),
            )
            merged[key] = item
        ...
        *,
        ...
        if manifest_path.exists():
            try:
                ...
                if courses and not replace_courses and existing.get("courses"):
                    manifest["courses"] = ManifestWriter._merge_course_lists(
                        existing.get("courses") or [], courses
                    )
                elif not courses and existing.get("courses"):
                    manifest["courses"] = existing["courses"]
                if not uni_pages and existing.get("uni_pages"):
                    manifest["uni_pages"] = existing["uni_pages"]
...
class CoursePagesCleaner:
    ...
    def __init__(self, code_dir: Path):
        self.code_dir = code_dir.resolve()
        self.output_dir = resolve_output_dir(self.code_dir)
        self.university_name = UniversityNameResolver.resolve(self.code_dir)
        self.markdown_cleanup = CourseMarkdownCleanupBridge(self.code_dir)
    ...
    def _delete_markdown_for_source(
        ...
        *,
        ...
        deleted = False
        for md_path in iter_course_markdown(courses_out):
            try:
                text = md_path.read_text(encoding="utf-8", errors="replace")
            except OSError:
                continue
            if f"source_html: {html_rel}" in text or f"source_url: {source_url}" in text:
                md_path.unlink()
                deleted = True
        return deleted
    ...
    def run(
        ...
        *,
        ...
        courses_out = clean_root / courses_subdir
        courses_out.mkdir(parents=True, exist_ok=True)

        entries = CourseEntryCatalog(self.code_dir).read_entries()
        url_levels = load_url_levels(self.output_dir)
        classifier = StudyLevelClassifier.from_code_dir(self.code_dir)
        if urls is not None:
            wanted = {normalize_url(url) for url in urls if (url or "").strip()}
            entries = [
                (course_url, html_path)
                for course_url, html_path in entries
                if normalize_url(course_url) in wanted
            ]
        if filter_levels:
            ...
            for course_url, html_path in entries:
                ...
                if any(level in allowed for level in found):
                    filtered.append((course_url, html_path))
            entries = filtered
        if limit is not None:
            entries = entries[:limit]
        if not entries:
            raise ValueError(
                f"No HTML files in {COURSE_PAGES_DIR}/ and no {COURSE_PAGE_MAP_CSV}. "
                f"Run download first or ensure {COURSE_URLS_CSV} URLs were downloaded."
            )
        ...
        clean_config = CleanConfigLoader.load(self.code_dir)
        course_filter = CourseTypeFilter.from_code_dir(self.code_dir)
        excluded_count = 0
        level_counts: dict[str, int] = {}
        warnings: list[CleanWarning] = []

        print(f"Cleaning {len(entries)} course pages...")
        if clean_config.blocks:
            print(
                f"COURSE_CLEAN_BLOCKS={len(clean_config.blocks)} block(s), "
                f"engine={clean_config.engine}"
            )
        else:
            print("COURSE_CLEAN_BLOCKS not set — using generic main-content extraction")
        for course_url, html_path in entries:
            html_rel = html_path.relative_to(self.output_dir).as_posix()
            raw_html = html_path.read_text(encoding="utf-8", errors="replace")
            source_url = HtmlSourceExtractor.extract_source_url(raw_html) or course_url

            if course_filter.should_exclude_html(raw_html, url=source_url or course_url):
                excluded_count += 1
            ...
            slug_base = course_slug_from_url(source_url or html_path.stem)
            ...
            if clean_config.blocks:
                ...
                markdown = GenericMarkdownBuilder.from_html(raw_html)
            markdown = self.markdown_cleanup.cleanup_course(markdown)

            for study_level in study_levels:
                folder = folder_for_level(study_level)
                key = (folder, slug_base)
                if key in used_keys:
                    continue
                used_keys.add(key)
                year_folder = intake_year_folder_from_stem(slug_base)
                level_dir = courses_out / folder
                if year_folder:
                    level_dir = level_dir / year_folder
                level_dir.mkdir(parents=True, exist_ok=True)
                output_path = level_dir / f"{slug_base}.md"
                ...
                relative_md = clean_course_md_relative_path(
                    folder,
                    slug_base,
                    clean_dir=CLEAN_DIR,
                ...
                manifest["courses"].append(
                    {
                        "course_url": course_url or source_url,
                        "source_html": html_rel,
                ...
                level_counts[folder] = level_counts.get(folder, 0) + 1

        if excluded_count:
            print(f"Excluded {excluded_count} short-course/part-time pages")
        warnings_path = CleanWarningsWriter.write(self.output_dir, warnings)
        if warnings:
            print(f"Wrote {len(warnings)} cleaning warning(s) to {warnings_path}")
        else:
            print(f"Wrote {warnings_path} (no warnings)")
        print(f"Wrote {len(manifest['courses'])} course markdown files to {courses_out}")
...
class UniReqPagesCleaner:
    ...
    def run(self) -> list[dict]:
        uni_req_dir = UniReqDirResolver.resolve(self.code_dir)
        if uni_req_dir is None:
            return []

        html_files = sorted(uni_req_dir.glob("*.html"))
        if not html_files:
            return []
        ...
        md_to_role = {md: role for role, md in UNI_MD_BY_ROLE.items()}
        print(f"Cleaning {len(html_files)} uni_req page(s) from {uni_req_dir}...")

        for html_path in html_files:
            html_rel = UniReqDirResolver.source_html_rel(html_path, self.code_dir)
            raw_html = html_path.read_text(encoding="utf-8", errors="replace")
            source_url = HtmlSourceExtractor.extract_source_url(raw_html)
            if not source_url:
                source_url = (
                    self.markdown_cleanup.uni_source_url(
                        raw_html, html_stem=html_path.stem
                    )
                    or ""
                )
            md_name = uni_md_output_name(html_path.stem)
            slug = Path(md_name).stem
            markdown = GenericMarkdownBuilder.from_html(raw_html)
            markdown = self.markdown_cleanup.cleanup_uni(
                markdown,
                html_stem=html_path.stem,
                raw_source=raw_html,
            )
            output_path = uni_out / md_name
            output_path.write_text(
                ManifestWriter.build_frontmatter(
                    source_html=html_rel,
                    source_url=source_url,
            ...
            manifest_pages.append(
                {
                    "source_html": html_rel,
                    "source_url": source_url,
                    "clean_md": f"{CLEAN_DIR}/uni/{md_name}",
                    "page_type": md_to_role.get(md_name, slug),
                }
            )

        print(f"Wrote {len(manifest_pages)} uni markdown files to {uni_out}")
...
class CleaningOrchestrator:
    ...
    def run_courses(
        ...
        *,
        ...
        university_name = UniversityNameResolver.resolve(self.code_dir)
        course_manifest: dict = {"courses": []}
        try:
            course_manifest = CoursePagesCleaner(self.code_dir).run(
                limit=limit,
                urls=urls,
            ...
            if "No HTML files" not in str(exc):
                raise
            print(f"Skipping course pages: {exc}")

        subset = urls is not None or bool(filter_levels) or limit is not None
        ManifestWriter.merge(
            self.code_dir,
            course_manifest.get("courses", []),
            [],
        ...
        if not course_manifest.get("courses"):
            raise ValueError(
                f"Nothing to clean: no HTML in {COURSE_PAGES_DIR}/"
            )

        manifest_path = self.output_dir / CLEAN_DIR / "manifest.json"
        print(f"Wrote {manifest_path}")
        return {"courses": course_manifest.get("courses", []), "uni_pages": []}

    def run_uni(self) -> dict:
        university_name = UniversityNameResolver.resolve(self.code_dir)
        uni_pages = UniReqPagesCleaner(self.code_dir).run()
        if not uni_pages:
            raise ValueError(
                f"Nothing to clean: no HTML in {UNI_REQ_DIR}/"
            )
        ...
        manifest_path = self.output_dir / CLEAN_DIR / "manifest.json"
        print(f"Wrote {manifest_path}")
        return {"courses": [], "uni_pages": uni_pages}
    ...
    def run(
        ...
        *,
        ...
        if not course_manifest.get("courses") and not uni_pages:
            raise ValueError(
                f"Nothing to clean: no HTML in {COURSE_PAGES_DIR}/ or {UNI_REQ_DIR}/"
            )

        manifest_path = self.output_dir / CLEAN_DIR / "manifest.json"
        print(f"Wrote {manifest_path}")
        return {"courses": course_manifest.get("courses", []), "uni_pages": uni_pages}
...
class CoursePagesPipeline:
    ...
    def __init__(self, code_dir: Path, config: dict[str, str]):
        self.code_dir = code_dir.resolve()
        self.output_dir = resolve_output_dir(self.code_dir)
        self.config = config
    ...
    def run(
        ...
        *,
        ...
        if not clean_only and not clean_uni_only and not clean_all:
            download_course_pages(
                self.code_dir,
                self.config,
                fresh=fresh,
        ...
        orchestrator = CleaningOrchestrator(self.code_dir)
        if clean_uni_only:
...
def clean_course_pages(work_dir: Path, *, limit: int | None = None) -> dict:
    return CleaningOrchestrator(work_dir).run_courses(limit=limit)


def clean_uni_req_pages(work_dir: Path) -> list[dict]:
    return CleaningOrchestrator(work_dir).run_uni()["uni_pages"]


def run_cleaning(work_dir: Path, *, limit: int | None = None, uni: bool = False) -> dict:
    orchestrator = CleaningOrchestrator(work_dir)
    if uni:
        return orchestrator.run_uni()
    return orchestrator.run_courses(limit=limit)
...
def download_and_clean_course_pages(
    ...
    *,
    ...
    return CoursePagesPipeline(work_dir, config).run(
        fresh=fresh,
        limit=limit,
...
def resolve_url_subset(
    ...
    *,
    ...
    """Return (explicit_urls_or_None, study_levels_or_None) for download/clean filters."""
    collected: list[str] = []
    if urls:
        collected.extend(urls)
    if urls_file is not None:
        collected.extend(read_urls_file(urls_file))
    collected = unique_urls(collected)
    levels = parse_study_levels(filter_levels) if filter_levels else []
    if collected:
        return collected, levels or None
    if levels:
        records = urls_for_levels(load_url_levels(output_dir), levels)
        return unique_urls([row["course_url"] for row in records]), levels
    return None, None
...
class CoursePagesCLI:
    ...
    def build_arg_parser() -> argparse.ArgumentParser:
        parser = argparse.ArgumentParser(
            description="Download course HTML pages and clean them into markdown."
        )
        parser.add_argument(
            "--fresh",
            action="store_true",
            help="Re-download all URLs (clears download progress)",
        )
        parser.add_argument(
            "--limit",
            type=int,
            default=None,
            help="Process only the first N course URLs (download and/or clean, for testing)",
        )
        parser.add_argument(
            "--download-only",
            action="store_true",
        ...
        parser.add_argument(
            "--clean-only",
            action="store_true",
            help="Clean existing course HTML only; skip download and uni_req",
        )
        parser.add_argument(
            "--clean-uni-only",
            action="store_true",
            help="Clean uni_req HTML only; skip download and course pages",
        )
        parser.add_argument(
            "--clean-all",
            action="store_true",
            help="Clean both course pages and uni_req; skip download",
        )
        parser.add_argument(
            "--url",
            action="append",
        ...
        parser.add_argument(
            ...
            type=Path,
            default=None,
            help="JSON/CSV/text file of course URLs to process",
        )
        parser.add_argument(
            "--study-level",
            action="append",
            default=[],
            metavar="LEVEL",
            help="Restrict to a study level (repeatable): foundation, undergraduate, postgraduate, postgraduate_research",
        )
    ...
    def main(code_dir: Path | None = None) -> int:
        args = CoursePagesCLI.build_arg_parser().parse_args()

        clean_modes = sum(
            1 for flag in (args.clean_only, args.clean_uni_only, args.clean_all) if flag
        )
        if clean_modes > 1:
            print(
                "Error: --clean-only, --clean-uni-only, and --clean-all are mutually exclusive.",
                file=sys.stderr,
            )
            return 1

        if args.download_only and clean_modes:
            print(
                "Error: --download-only cannot be combined with a clean mode.",
                file=sys.stderr,
            )
            return 1

        work_dir = resolve_work_dir(code_dir if code_dir is not None else args.code_dir)
        try:
            urls, filter_levels = resolve_url_subset(
                resolve_output_dir(work_dir),
                urls=args.url,
                urls_file=args.urls_file,
                filter_levels=args.study_level,
            )
            config = load_strategy_config(work_dir)
            CoursePagesPipeline(work_dir, config).run(
                fresh=args.fresh,
                limit=args.limit,
            ...
            try:
                logger = ScrapeLogger(resolve_output_dir(work_dir))
