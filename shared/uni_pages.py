...
def uni_md_output_name(html_stem: str) -> str:
    """Map uni_req HTML stem to generic clean/uni/*.md filename."""
    key = html_stem.lower().strip()
    if key in HTML_STEM_TO_MD:
        return HTML_STEM_TO_MD[key]
    for html_name, md_name in zip(UNI_HTML_BY_ROLE.values(), UNI_MD_BY_ROLE.values()):
        if key == Path(html_name).stem.lower():
            return md_name
    return f"{slug_from_stem(html_stem)}.md"
