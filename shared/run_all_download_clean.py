...
def progress_path(repo_root: Path) -> Path:
    return repo_root / PROGRESS_NAME
...
def save_progress(repo_root: Path, progress: dict) -> None:
    progress["updated_at"] = utc_now()
    progress_path(repo_root).write_text(
        json.dumps(progress, indent=2),
        encoding="utf-8",
    )


def discover_universities(repo_root: Path) -> list[Path]:
    found: list[Path] = []
...
def main() -> int:
    ...
    if not DOWNLOAD_SCRIPT.is_file():
        print(f"Error: missing {DOWNLOAD_SCRIPT}", file=sys.stderr)
        return 1

    repo_root = args.repo_root.resolve()
    exclude = set() if args.include_done else set(DEFAULT_EXCLUDE)
    ...
    for uni_dir in discover_universities(repo_root):
        name = uni_dir.name
        ...
        if not (uni_dir / "code").is_dir():
            ...
            continue
        if count_course_urls(uni_dir) <= 0 and not args.clean_only:
    ...
    for uni_dir in targets:
        urls = count_course_urls(uni_dir)
