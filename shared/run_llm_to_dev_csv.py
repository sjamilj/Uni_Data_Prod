...
class LlmToDevCsvPipeline:
    """Run LLM extract, normalize, and dev CSV export for one university."""
    ...
    def resolve_university_code_dir(self, args: argparse.Namespace) -> Path:
        if args.code_dir:
            return resolve_code_dir(Path(args.code_dir))
        if args.university:
            return resolve_code_dir(self.repo_root / args.university / "code")
        return resolve_code_dir(Path.cwd())

    def run_step(self, label: str, command: list[str], cwd: Path) -> None:
        print()
        print(f"==> {label}")
        print(" ".join(command))
    ...
    def run(self, args: argparse.Namespace) -> int:
        code_dir = self.resolve_university_code_dir(args)
        output_dir = resolve_output_dir(code_dir)
        python = self.utc_python()
        extract_script = self.shared_dir / "llm_extract.py"
        normalize_script = self.shared_dir / "normalize_admission_data.py"
        export_script = self.shared_dir / "export_dev_courses.py"
        ...
        if not args.skip_extract and md_count == 0:
            ...
            return 1
        if not args.skip_extract and not self.ollama_ok(host):
        ...
        if not args.skip_extract:
            ...
            self.run_step("Phase 3 - LLM extract", extract_cmd, self.repo_root)
...
class LlmToDevCsvCLI:
    """Command-line entry point for the LLM-to-dev-CSV pipeline."""
    ...
    def main(cls) -> int:
        args = cls.parse_args()
        return LlmToDevCsvPipeline().run(args)


# Backward-compatible module-level aliases
utc_python = LlmToDevCsvPipeline.utc_python
ollama_ok = LlmToDevCsvPipeline.ollama_ok
run_step = LlmToDevCsvPipeline().run_step
resolve_university_code_dir = LlmToDevCsvPipeline().resolve_university_code_dir
parse_args = LlmToDevCsvCLI.parse_args
main = LlmToDevCsvCLI.main


if __name__ == "__main__":
    raise SystemExit(LlmToDevCsvCLI.main())
