...
def extra_clean_course_markdown(
    ...
    *,
    ...
    for env_key in (
        extra_clean_env_key_for_level(study_level),
        _EXTRA_CLEAN_REMOVE_SECTIONS_KEY,
    ):
...
def _select_levels(courses_dir: Path, requested: list[str]) -> list[str]:
    if requested:
    ...
    *,
    ...
    for path in iter_course_markdown(courses_dir):
        ...
        for _ in range(max(1, passes)):
            cleaned_body = extra_clean_course_markdown(
                cleaned_body,
                code_dir=code_dir,
...
def main(argv: list[str] | None = None) -> int:
    ...
    levels = _select_levels(courses_dir, [level for group in args.level for level in group])
    if not levels:
    ...
    updated, total = apply_extra_clean_to_dir(
        courses_dir,
        code_dir,
        levels=levels,
        passes=args.passes,
