...
class UniCleanConfigScaffolder:
    """Apply per-university course clean profiles to code/.env and code/ENV.MD."""

    CLEAN_KEYS = CLEAN_KEYS
    STANDARD_MARKDOWN_REMOVE = STANDARD_MARKDOWN_REMOVE
    DEFAULT_STRIP = DEFAULT_STRIP

    def __init__(self, repo_root: Path | None = None, profiles_path: Path | None = None) -> None:
        self.repo_root = repo_root or REPO_ROOT
        self.profiles_path = profiles_path or PROFILES_PATH
        self.template_cleanup = self.repo_root / "_university_template" / "code" / "course_markdown_cleanup.py"

    def load_profiles(self) -> dict:
        return json.loads(self.profiles_path.read_text(encoding="utf-8"))

    @staticmethod
    def strip_clean_keys(text: str) -> str:
    ...
    def uni_req_urls(self, uni_dir: Path) -> list[str]:
        urls: list[str] = []
    ...
    @staticmethod
    def fix_paths(text: str, uni_dir: Path) -> str:
    ...
    def build_clean_section(self, profile: dict, uni_dir: Path) -> str:
        title = profile.get("title", "")
        expand = "true" if profile.get("expand_tabs", True) else "false"
        blocks = profile.get("blocks") or ["Course overview :: #main-content"]
        strip = list(dict.fromkeys([*self.DEFAULT_STRIP, *profile.get("strip", [])]))
        uni_req = self.uni_req_urls(uni_dir)
        if not uni_req:
            uni_req = [
                "bangladesh-entry ::",
        ...
        lines = [
            ...
            f"COURSE_CLEAN_BLOCKS={self.quote_block(blocks)}",
            "",
            "# Noise removed inside each matched block before markdown conversion",
            f"COURSE_CLEAN_STRIP_WITHIN={self.quote_block(strip)}",
            "",
            f"COURSE_CLEAN_EXPAND_TABS={expand}",
            "",
            "# --- Course markdown cleanup (shared/course_markdown_cleanup.py) ---",
            f"COURSE_MARKDOWN_REMOVE_SECTIONS={self.quote_block(self.STANDARD_MARKDOWN_REMOVE.splitlines())}",
            "",
            "UNI_REQ_SOURCE_URLS=" + self.quote_block(uni_req),
            "",
        ]
        return "\n".join(lines)

    def scaffold_university(self, uni_dir: Path, profiles: dict, *, dry_run: bool = False) -> str:
        name = uni_dir.name
        ...
        if profile.get("skip"):
            if not env_file.is_file() and env_md.is_file():
                if not dry_run:
                    env_file.write_text(self.fix_paths(env_md.read_text(encoding="utf-8"), uni_dir), encoding="utf-8")
            if not cleanup_py.is_file() and not dry_run:
                shutil.copy2(self.template_cleanup, cleanup_py)
            return "skip (already configured)"

        base = self.strip_clean_keys(self.fix_paths(env_md.read_text(encoding="utf-8"), uni_dir))
        updated = base + self.build_clean_section(profile, uni_dir)
    ...
    def run(self, *, dry_run: bool = False, universities: list[str] | None = None) -> int:
        profiles = self.load_profiles()
        targets: list[Path] = []
        if universities:
            for name in universities:
                targets.append(self.repo_root / name)
        else:
        ...
        for uni_dir in targets:
            status = self.scaffold_university(uni_dir, profiles, dry_run=dry_run)
...
def main() -> int:
    ...
    return UniCleanConfigScaffolder().run(dry_run=args.dry_run, universities=args.university)


# Backward-compatible module-level aliases
_load_profiles = UniCleanConfigScaffolder().load_profiles
_strip_clean_keys = UniCleanConfigScaffolder.strip_clean_keys
_quote_block = UniCleanConfigScaffolder.quote_block
_uni_req_urls = UniCleanConfigScaffolder().uni_req_urls
_fix_paths = UniCleanConfigScaffolder.fix_paths
_build_clean_section = UniCleanConfigScaffolder().build_clean_section
scaffold_university = UniCleanConfigScaffolder().scaffold_university
