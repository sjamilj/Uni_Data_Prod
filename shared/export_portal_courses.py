...
def portal_output_path(repo_root: Path, uni_name: str) -> Path:
    return repo_root / uni_name / f"{uni_name}_portal.csv"
...
def export_portal_courses(
    ...
    *,
    ...
    uni_names = read_uni_list(uni_list_csv)
    if only_uni:
        only_uni = only_uni.strip()
        if only_uni not in uni_names:
            raise ValueError(f"{only_uni!r} not found in {uni_list_csv}")
        uni_names = [only_uni]

    fieldnames, courses_by_uni = load_courses_by_uni(uk_course_csv)
    written: list[Path] = []
    missing_dirs: list[str] = []
    empty_unis: list[str] = []

    for uni_name in uni_names:
        uni_dir = repo_root / uni_name
        ...
        output_path = portal_output_path(repo_root, uni_name)
        write_portal_csv(output_path, fieldnames, rows)
    ...
    if empty_unis:
        print(
            f"Warning: no rows in {uk_course_csv.name} for: "
            + ", ".join(empty_unis),
            file=sys.stderr,
        )
