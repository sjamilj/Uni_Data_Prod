...
def resolve_university_code_dir(name: str) -> Path:
    code_dir = _REPO_ROOT / name / "code"
    if not code_dir.is_dir():
        raise FileNotFoundError(f"University code folder not found: {code_dir}")
    return code_dir


def split_intake_years(output_dir: Path, *, dry_run: bool) -> tuple[int, int]:
...
def update_manifest(output_dir: Path, *, dry_run: bool) -> int:
...
def retrofit_university(name: str, *, dry_run: bool) -> None:
    ...
    print(f"\n=== {label}{name} ===")
    moved, skipped = split_intake_years(output_dir, dry_run=dry_run)
    print(f"  moved: {moved}, skipped: {skipped}")
    manifest_updates = update_manifest(output_dir, dry_run=dry_run)
...
def main() -> int:
    ...
    retrofit_university(args.university, dry_run=args.dry_run)
    return 0
