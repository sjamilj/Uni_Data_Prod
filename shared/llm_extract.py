    ...
    *,
    ...
    def extract_stage1_fields_from_md(body: str) -> dict[str, str]:
        ...
        if not fields.get('courseDuration'):
            ...
            if course_length_match:

                fields['courseDuration'] = course_length_match.group(1).strip()
        ...
        if not fields.get('tuitionFee'):
            ...
            typical_fees_match = re.search(

                r'(?:-\s*)?\*\*Typical annual fees:\*\*\s*(?:Overseas\s*)?£([\d,]+)',
        ...
        if not ielts_match:
            ...
            ielts_match = re.search(
                ...
                r'IELTS with an overall score of\s+([\d.]+)\s+with no less than\s+([\d.]+)',
...
class Stage2Enricher:
    ...
    *,
    ...
    def parse_english_from_course_markdown(course_body: str) -> dict[str, object]:
        ...
        if not descriptions and (section.strip() if section else source.strip()):
