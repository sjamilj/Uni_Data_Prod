...
def load_uni_sections(output_dir: Path) -> dict[str, str]:
    ...
    return {
        role: load_uni_section(output_dir, filename)
...
def infer_degree_name(course_name: str) -> str:
    leading = re.match(
        r"^(BSc|BA|BBA|BEng|LLB|MSc|MA|MBA|MFA|PhD|MRes|PGDip|PGCert|FdSc|Fd|CertHE|MEng|BM BS)\b",
        course_name,
        re.I,
    )
    if leading:
    ...
    match = re.search(
        r"\b(BSc|BA|BBA|BEng|LLB|MSc|MA|MBA|MFA|PhD|MRes|PGDip|PgCert|FdSc|MEng)\b",
        course_name,
        re.I,
    )
    if match:
...
def extract_international_fees_section(body: str) -> str:
    match = re.search(
        r"### International students\s*(.*?)(?:--END--|\Z)",
        body,
        re.S | re.I,
    )
...
def parse_international_fee_options(section: str) -> list[dict[str, str]]:
    ...
    while index < len(lines):
        if re.fullmatch(r"-\s*Full Time", lines[index], re.I):
            duration = ""
            fee = ""
...
def normalize_short_month_date(text: str) -> str:
    ...
    match = re.match(r"^([A-Za-z]{3,})\s+(\d{4})$", (text or "").strip())
    if not match:
...
def extract_stage1_fields_from_md(body: str) -> dict[str, str]:
    ...
    if start_date_match:
        fields["intakeInfo"] = start_date_match.group(1).strip()
    if not fields.get("intakeInfo"):
        start_block_match = re.search(
            r"\*\*Start date\*\*\s*\n+\s*([^\n#]+)",
            body,
            re.I,
        )
        if start_block_match:
            fields["intakeInfo"] = start_block_match.group(1).strip()
    if not fields.get("intakeInfo"):
        starting_match = re.search(r"Starting:\s*([^\n]+)", body, re.I)
        if starting_match:
            fields["intakeInfo"] = normalize_short_month_date(starting_match.group(1))

    duration_match = re.search(r"\*\*Duration:\*\*\s*(.+)", body, re.I)
    if duration_match:
    ...
    if intl_section:
        ...
        if primary_fee:
            ...
            if not fields.get("tuitionFee"):
                ...
                fee_num = re.search(r"£([\d,]+)", fee_raw)
                fields["tuitionFee"] = (
                    fee_num.group(1).replace(",", "") if fee_num else fee_raw
                )
        if not fields.get("currency") and (fields.get("tuitionFee") or "£" in intl_section):
            fields["currency"] = "GBP"

    ielts_match = re.search(
        r"IELTS\s+([\d.]+)\s+overall\s+with\s+no\s+less\s+than\s+([\d.]+)\s+in\s+each\s+band",
        body,
        re.I,
    )
    if ielts_match:
...
def extract_expected_from_md(
    ...
    *,
    ...
    study_mode_match = re.search(r"- Study mode\s+([^\n]+)", body)
    ...
    expected: dict[str, object] = {
        ...
        "studyMode": study_mode_match.group(1).strip() if study_mode_match else "",
        "award": award_match.group(1).strip() if award_match else "",
        "intakeInfo": hints.get("intakeInfo", ""),
        "courseDuration": hints.get("courseDuration", "") or (
            primary_fee["courseDuration"] if primary_fee else ""
        ),
        "tuitionFee": hints.get("tuitionFee", "") or (
            primary_fee["tuitionFee"] if primary_fee else ""
        ),
        "currency": hints.get("currency", "") or ("GBP" if primary_fee or "£" in intl_section else ""),
        "ieltsMinOverall": hints.get("ieltsMinOverall", ""),
        "ieltsMinSection": hints.get("ieltsMinSection", ""),
    }
...
def attach_extract_artifacts(row: dict[str, str], output_dir: Path) -> dict[str, str]:
    row.update(
        extract_artifact_relpaths(
            output_dir,
            row.get("courseUrlExternal") or "",
            row.get("study_level") or "",
        )
    )
...
def write_course_index(output_dir: Path, rows: list[dict[str, str]]) -> Path:
    ...
    with output_path.open("w", newline="", encoding="utf-8-sig") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=COURSE_INDEX_COLUMNS,
            delimiter=",",
...
def read_course_index_csv(output_dir: Path) -> list[dict[str, str]]:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"{path} not found — run: python llm_extract.py --build-index"
        )
...
def index_row_to_entry(row: dict[str, str]) -> dict[str, str]:
    ...
    return {
        "uniName": row.get("uniName", "").strip(),
        "courseName": row.get("courseName", "").strip(),
        "degreeName": row.get("degreeName", "").strip(),
        "course_url": row.get("courseUrlExternal", "").strip(),
        "courseUrlExternal": row.get("courseUrlExternal", "").strip(),
        "md_file": md_file,
...
def dedupe_course_index_rows(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for (level, course_url) in sorted(grouped):
        ...
        deduped.append(
            min(
                rows_for_url,
                key=lambda row: canonical_md_priority(
                    Path(row.get("md_file", "")).name,
                    course_url,
                ),
            )
        )
...
def build_course_index(code_dir: Path) -> Path:
    ...
    for md_path in canonical_paths:
        ...
        if not course_url:
            continue
        course_name = infer_course_name(body, course_url)
        degree_name = infer_degree_name(course_name)
        uni_name = meta.get("university", university_name).strip() or university_name
        study_level = study_level_from_markdown(
            md_path,
            meta,
            courses_dir=courses_dir,
            course_url=course_url,
        ...
        expected_from_md = extract_expected_from_md(
            body,
            course_name=course_name,
            course_url=course_url,
            degree_name=degree_name,
        )
        stage1_output, stage2_output, output_json = detect_stage_outputs(
            output_dir, course_url, study_level
        )
        rows.append(
            attach_extract_artifacts(
                {
                    "uniName": uni_name,
                    "courseName": course_name,
                    "degreeName": degree_name,
                    "courseUrlExternal": course_url,
                    "md_file": relative_course_md(md_path, courses_dir),
                    "study_level": study_level,
                    "expectedFromMd": expected_from_md,
    ...
    if skipped:
        print(
            f"Skipped {skipped} duplicate intake/URL variant(s) "
            f"({total_md_files} markdown files -> {len(rows)} courses)",
            flush=True,
        )
...
def course_index_is_stale(output_dir: Path) -> bool:
    ...
    if not index_path.exists():
        return True
    index_md = {
        row.get("md_file", "").strip()
        for row in read_course_index_csv(output_dir)
        if row.get("md_file", "").strip()
    }
...
def ensure_course_index_synced(code_dir: Path) -> Path:
    ...
    if course_index_is_stale(output_dir):
        print(
            "courses.csv out of sync with clean/courses — rebuilding index",
            flush=True,
        )
...
def entries_from_presetup_clean(output_dir: Path) -> list[dict[str, str]]:
    ...
    for md_path in iter_course_markdown(courses_dir):
        ...
        study_level = url_level.get(key) or study_level_from_markdown(
            md_path,
            meta,
            courses_dir=courses_dir,
        ...
        entries.append(
            {
                "uniName": "",
                "courseName": infer_course_name(body, source_url),
                "degreeName": "",
                "course_url": source_url,
                "courseUrlExternal": source_url,
                "md_file": rel,
...
def normalize_duration_months(value: str) -> str:
    ...
    if not value:
        return ""
    match = re.search(r"(\d+)\s*months?\b", value, re.I)
    if match:
        return match.group(1)
    match = re.search(r"(\d+)\s*years?\b", value, re.I)
    if match:
...
def canonicalize_requirement_degree(degree: str) -> str:
    normalized = re.sub(r"\s+", " ", (degree or "").strip()).lower()
...
def extract_grade_from_requirement_text(text: str) -> str:
    ...
    if not value:
        return ""
    cgpa = re.search(r"CGPA\s*([\d.]+)", value, re.I)
    if cgpa:
        return f"CGPA {cgpa.group(1)}"
    pct = re.search(r"(\d+)\s*%", value)
    if pct:
        return f"{pct.group(1)}%"
    gpa = re.search(r"GPA\s*(?:of\s+)?([\d.]+)", value, re.I)
    if gpa:
        return f"GPA {gpa.group(1)}"
    score = re.search(r"\b(\d{2,3})\b", value)
    if score:
...
def filter_academic_metadata(metadata: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for item in metadata:
        ...
        if not isinstance(description, list) or not description:
            continue
        # Empty subtitle + IELTS-like text → English Requirement
        if not subtitle and any("ielts" in str(d).lower() or "overall" in str(d).lower() for d in description):
            subtitle = "english requirement"
            # Ensure IELTS prefix when model omitted it
            fixed_desc: list[str] = []
            for desc in description:
                text = str(desc).strip()
                if text and not re.search(r"\bIELTS\b", text, re.I) and re.search(r"\d\.\d", text):
                    text = f"IELTS {text}"
...
SCHOLARSHIP_JSON_LEVEL_ALIASES = {
    "foundation": ("undergraduate", "foundation"),
    "undergraduate": ("undergraduate",),
    "postgraduate": ("postgraduate",),
}
...
def filter_stage1_entry_description(text: str) -> bool:
    ...
    skip_patterns = (
        r"^see\s+",
        r"see information about application",
        r"\bielts\b",
        r"\btoefl\b",
        r"\bpearson pte\b",
        r"\bpte academic\b",
        r"english is not your first language",
        r"english language requirement",
        r"proficient in english",
        r"command of english",
        r"previously been taught in english",
        r"sufficient command of english",
        r"taught in english",
        r"check the section",
        r"please contact",
        r"contact aru",
        ...
        r"mailto:",
        r"guide only",
        r"treat everyone as an individual",
        r"should still consider applying",
        r"our decision will be based",
        r"studying for other qualifications should contact",
        r"^international students$",
        r"^standard entry requirements$",
        r"^requirement \d+$",
        r"^applicants must have:\s*$",
        r"did not achieve our required grades",
        r"life/work skills",
        r"whole application",
        r"personal circumstances",
        r"equivalent english",
        r"welcome applications from international",
        r"computer and reliable internet",
        r"digital audition",
        r"portfolios? and auditions",
        r"clearing auditions",
    )
...
def extract_uk_offer_grades(text: str) -> set[str]:
    grades: set[str] = set()
    for match in re.finditer(r"\b([A-D]{3})(?:\s*[-–]\s*([A-D]{3}))?\b", text, re.I):
...
def score_english_program(program_name: str, haystack: str) -> int:
    ...
    for keywords, hint in _ENGLISH_PROGRAM_HINTS:
        if hint in program_lower and any(keyword in haystack_lower for keyword in keywords):
            score += 10
    for token in re.split(r"[\s:,&/()\-]+", program_lower):
...
def select_english_json_program(
    ...
    *,
    ...
    scored = sorted(
        ((score_english_program(str(item.get("ProgramName", "") or ""), haystack), item) for item in candidates),
    ...
    for level in entry_json.get("studyLevels", []):
        ...
        for program in level.get("programs", []):
            if not isinstance(program, dict):
                continue
            descriptions = normalize_description_list(program.get("description"))
            return filter_bangladesh_descriptions_for_course(
                descriptions,
...
def extract_english_json_descriptions(
    ...
    *,
    ...
    if not program:
        return []
    return normalize_description_list(program.get("description"))
...
def finalize_academic_requirements_metadata(
    ...
    *,
    ...
    entry_descriptions: list[str] = list(extract_stage1_entry_descriptions(stage1_json))

    seen = set(entry_descriptions)
    for line in extract_bangladesh_json_descriptions(
        bangladesh_source,
        course_level,
        course_text=course_text,
    ):
    ...
    if not english_descriptions:
        for item in normalize_metadata_array(metadata):
            if str(item.get("subtitle", "")).strip().lower() == "english requirement":
                english_descriptions = normalize_description_list(item.get("description"))
                break
    if not english_descriptions:
        ...
        if overall:
            english_descriptions = [
                f"IELTS {overall} overall with no element below {section}"
                if section
...
def parse_bangladesh_requirements(
    ...
    *,
    ...
    for source in (entry_content, uni_content):
        ...
        if isinstance(entry_json, dict):
            json_requirements = normalize_requirements_list(
                parse_bangladesh_json_requirements(entry_json, course_level),
                course_level=course_level,
            )
    ...
    if course_level == "foundation":
        cgpa = re.search(r"CGPA\s*([\d.]+)", text, re.I)
    ...
    if course_level == "undergraduate":
        hsc = re.search(r"(?:Intermediate/)?HSC[^\d]*CGPA\s*([\d.]+)", text, re.I)
        if hsc:
            requirements.append({"degree": "HSC", "grade": f"CGPA {hsc.group(1)}"})
        diploma = re.search(
            r"(?:polytechnic\s+)?Diploma[^\d]*(?:with\s+grades\s+above\s+)?(\d+)\s*%",
            text,
            re.I,
        )
        if diploma:
    ...
    if course_level == "postgraduate":
        bachelor = re.search(
            r"bachelor[^\d]*(\d+)\s*%|bachelor[^\d]*CGPA\s*([\d.]+)",
            text,
            re.I,
        )
        if bachelor:
            grade = (
                f"CGPA {bachelor.group(2)}"
                if bachelor.group(2)
                else f"{bachelor.group(1)}%"
            )
            requirements.append({"degree": "BSc", "grade": grade})
        master = re.search(
            r"master[^\d]*(\d+)\s*%|master[^\d]*CGPA\s*([\d.]+)",
            text,
            re.I,
        )
        if master:
            grade = (
                f"CGPA {master.group(2)}"
                if master.group(2)
                else f"{master.group(1)}%"
            )
...
def select_english_row_label(course_name: str, course_level: str, english_content: str) -> str:
    ...
    # Named course rows (non-standard list tables)
    for line in english_content.splitlines():
        ...
        if len(cells) < 2:
            continue
        label = cells[0]
        if label.lower() == course_name.lower() or (
            len(label) > 8 and label.lower() in name_lower
        ):
            if re.search(r"\d\.\d|overall|minimum", cells[1], re.I):
                return label
    ...
    if course_level == "postgraduate" and NON_STANDARD_PG_ENGLISH_ROW.lower() in english_content.lower():
        # Only if course appears under non-standard programmes list
        list_match = re.search(
            r"List of non-standard programmes(.*?)(?:\n##|\Z)",
            english_content,
            re.S | re.I,
        )
        if list_match:
...
def parse_english_test_scores(
    ...
    *,
    ...
    ielts = re.search(
        rf"{label_re}\s*\|\s*([\d.]+)\s+overall with no element below\s+([\d.]+)",
        english_content,
        re.I,
    )
    if ielts:
    ...
    if toefl_block:
        toefl = re.search(
            rf"{label_re}\s*\|\s*Minimum of\s*([\d.]+)\s*overall:\s*"
            rf"Reading\s*([\d.]+),\s*Listening\s*([\d.]+),\s*"
            rf"Speaking\s*([\d.]+)\s*(?:and|,)\s*Writing\s*([\d.]+)",
            toefl_block.group(0),
            re.I,
        )
        if toefl:
            ...
            scores["toeflMinSection"] = (
                str(int(min_section)) if min_section.is_integer() else str(min_section)
            )
        else:
            toefl_simple = re.search(
                rf"{label_re}\s*\|\s*(?:Minimum of\s*)?([\d.]+)\s*overall",
                toefl_block.group(0),
                re.I,
            )
            if toefl_simple:
    ...
    # Named-course IELTS-only tables (no PTE/TOEFL columns)
    if not scores["ieltsMinOverall"]:
        named = re.search(
            rf"\|\s*{label_re}\s*\|\s*(?:IELTS[^|]*?)?([\d.]+)\s+overall with no element below\s+([\d.]+)",
            english_content,
            re.I,
        )
        if named:
...
def enrich_english_parsed(
    ...
    *,
    ...
    fallback = parse_english_test_scores(
        english_content,
        course_name=course_name,
        course_level=course_level,
    )
    json_program = select_english_json_program(
        parse_uni_json_payload(english_content, "english-requirements") or [],
        course_level=course_level,
        course_name=course_name,
        course_body=course_body,
    )
    ...
    # Prefer Stage 1 IELTS when present (course page)
    ...
    meta = normalize_metadata_array(
        parsed.get("AcademicRequirementsMetaData"),
        default_subtitle="English Requirement",
    )
    ...
    return {
        "AcademicRequirementsMetaData": parsed.get("AcademicRequirementsMetaData", []),
        **{key: str(parsed.get(key, "") or "").strip() for key in ENGLISH_TEST_KEYS},
    }
...
def scholarship_study_level_matches(study_level: str, course_level: str) -> bool:
    ...
    tokens = [token.strip().lower() for token in re.split(r"[,/&]", study_level) if token.strip()]
...
def parse_scholarship_numeric_amount(item: dict) -> float:
    ...
    amount_numbers = [
        float(normalize_fee_numeric(match))
        for match in re.findall(r"£[\d,]+(?:\.\d+)?", amount_text)
        if normalize_fee_numeric(match)
    ]
    if amount_numbers:
        return max(amount_numbers)

    description = str(item.get("description", "") or "")
    total_match = re.search(r"£([\d,]+(?:\.\d+)?)\s+total\b", description, re.I)
    if total_match:
        return float(normalize_fee_numeric(total_match.group(1)))

    value_match = re.search(
        r"(?:scholarship value|value)\s*:\s*£([\d,]+(?:\.\d+)?)",
        description,
        re.I,
    )
    if value_match:
        return float(normalize_fee_numeric(value_match.group(1)))

    description_numbers = [
        float(normalize_fee_numeric(match))
        for match in re.findall(r"£([\d,]+(?:\.\d+)?)", description)
        if normalize_fee_numeric(match)
    ]
...
def select_scholarships_for_course(
    ...
    *,
    ...
    eligible = [
        ...
        if isinstance(item, dict)
        and scholarship_study_level_matches(
            str(item.get("scholarshipStudyLevel", "") or ""),
            course_level,
        )
    ]
    if not eligible:
    ...
    haystack = f"{course_name}\n{course_body}".casefold()
    is_mba_course = bool(re.search(r"\bmba\b", haystack))
    if is_mba_course:
        mba_only = [
            item
            for item in eligible
            if "mba" in str(item.get("scholarshipName", "") or "").casefold()
        ]
        if mba_only:
            return mba_only
    elif course_level == "postgraduate":
        eligible = [
            item
            for item in eligible
            if "mba" not in str(item.get("scholarshipName", "") or "").casefold()
        ]
    return eligible
...
def enrich_scholarship_parsed(
    ...
    *,
    ...
    eligible = select_scholarships_for_course(
        data,
        course_level=course_level,
        course_name=course_name,
        course_body=course_body,
...
def enrich_entry_parsed(
    ...
    *,
    ...
    course_text = f"{course_name}\n{course_body}".strip()
    descriptions: list[str] = list(extract_stage1_entry_descriptions(stage1_json))
    ...
    if descriptions:
        parsed["AcademicRequirementsMetaData"] = [
            {"subtitle": "Entry Requirements", "description": descriptions}
        ]
    else:
        parsed["AcademicRequirementsMetaData"] = []
    return parsed
...
def extract_entry_lines_from_course_markdown(course_body: str) -> list[str]:
    ...
    for raw_line in section.splitlines():
        ...
        if stripped.startswith("|") and "|" in stripped[1:]:
            ...
            continue
        if re.search(r"\bucas tariff points\b", stripped, re.I):
            add_line(stripped if stripped.endswith(".") else f"{stripped}.")
            continue
        if re.search(r"\bgcse\b", stripped, re.I) and re.search(
            r"grade|at grade|c/4|4 or above",
            stripped,
            re.I,
        ):
            add_line(stripped)
...
def _requirement_identity(req: dict[str, str]) -> tuple[str, str]:
    return (
        str(req.get("degree", "") or "").strip().lower(),
        str(req.get("grade", "") or "").strip().lower(),
    )
...
def patch_fees_metadata_international_fee(
    ...
    meta = normalize_metadata_array(fees_meta)
    patched = False
    for block in meta:
...
def assert_grounded(value: str, body: str) -> bool:
    ...
    if text in body:
        return True
    if fee_amount_in_markdown(text, body):
        return True
    digits = re.sub(r"[^\d.]", "", text.replace(",", ""))
    if digits and fee_amount_in_markdown(digits, body):
        return True
    tokens = re.findall(
        r"\d+(?:[./-]\d+)*|\b(?:January|February|March|April|May|June|July|"
...
def parser_hints_payload(hints: dict[str, str], course_body: str) -> dict[str, object]:
    ...
    for line in course_body.splitlines():
        ...
        if not stripped:
            continue
        if fee and "£" in stripped and fee_amount_in_markdown(fee, stripped):
            snippets.setdefault("tuitionFee", stripped)
        if hints.get("intakeInfo") and (
            stripped.lower().startswith("starting:") or "start date" in stripped.lower()
        ):
            snippets.setdefault("intakeInfo", stripped)
        if hints.get("courseDuration") and re.search(r"\d+\s*(?:year|month)", stripped, re.I):
...
def apply_parser_owned_stage1_fields(parsed: dict, hints: dict[str, str]) -> None:
    ...
    if hints.get("tuitionFee"):
        parsed["feesMetaData"] = patch_fees_metadata_international_fee(
            parsed.get("feesMetaData"),
            hints["tuitionFee"],
        )
...
def enrich_stage1_from_markdown(
    ...
    *,
    ...
    apply_parser_owned_stage1_fields(parsed, hints)
    ...
    if not extract_stage1_entry_descriptions(parsed):
        ...
        if not entry_lines:
            for line in course_body.splitlines():
                ...
                elif re.search(r"\bBCC\b|\bBBB\b|\bBBC\b|\bCCD\b|\bCDD\b|\bDDM\b|\bDMM\b", stripped):
                    if "A-level" in stripped or stripped.startswith("B/") or stripped.startswith("C/"):
                        entry_lines.append(stripped)
        if entry_lines:
            parsed["AcademicRequirementsMetaData"] = [
                {"subtitle": "Entry Requirements", "description": entry_lines}
            ]
    return parsed
...
def build_deterministic_row(
    ...
    *,
    ...
    row["tuitionFee"] = normalize_fee_numeric(str(stage1_json.get("tuitionFee", "") or ""))
    row["currency"] = str(stage1_json.get("currency", "") or "").strip() or "GBP"
    row["intakeInfo"] = normalize_intake(str(stage1_json.get("intakeInfo", "") or ""))
    row["courseDuration"] = normalize_duration_months(str(stage1_json.get("courseDuration", "") or ""))
...
def extract_llm_stage2_fields(
    ...
    *,
    ...
    extracted["requirements"] = normalize_requirements_list(
        extracted.get("requirements"),
        course_level=course_level,
    )
    extracted["AcademicRequirementsMetaData"] = filter_academic_metadata(
        normalize_metadata_array(extracted.get("AcademicRequirementsMetaData"))
    )
    extracted["scholarshipMetaData"] = normalize_metadata_array(
        extracted.get("scholarshipMetaData"),
        default_subtitle="Scholarships",
    )
    extracted["scholarshipName"] = str(extracted.get("scholarshipName", "") or "").strip()
    extracted["scholarshipAmount"] = normalize_fee_numeric(
        str(extracted.get("scholarshipAmount", "") or "")
    )
    ...
    if extracted["scholarshipType"].lower() == "percentage":
        ...
        # Model sometimes returns prose; coerce £/amount wording → Amount
...
def enrich_deposit_parsed(
    ...
    *,
    ...
    fees_meta = normalize_metadata_array(
        enriched.get("feesMetaData"),
        default_subtitle="Initial tuition Deposit",
    )
    enriched["feesMetaData"] = fees_meta

    meta_text = "\n".join(
        line
        for item in fees_meta
        for line in (item.get("description") or [])
        if isinstance(line, str)
    )
    explicit = extract_explicit_deposit_from_text(
        course_content,
        deposit_content,
        meta_text,
    )
...
def extract_deposit_stage2_fields(deposit_json: dict) -> dict[str, object]:
    fees_meta = normalize_metadata_array(
        deposit_json.get("feesMetaData"),
        default_subtitle="Initial tuition Deposit",
    )
    return {
        "initialDeposit": str(deposit_json.get("initialDeposit", "") or "").strip(),
        "feesMetaData": fees_meta,
    }
...
def merge_fees_metadata(
    ...
    if not deposit_meta:
        return merged
    existing_subtitles = {
        str(item.get("subtitle", "")).strip().lower() for item in merged
    }
...
def combine_stage2_llm_parts(
    ...
    metadata.extend(
        normalize_metadata_array(
            entry_json.get("AcademicRequirementsMetaData"),
            default_subtitle="Entry Requirements",
        )
    )
    metadata.extend(
        normalize_metadata_array(
            english_json.get("AcademicRequirementsMetaData"),
            default_subtitle="English Requirement",
        )
    )

    combined: dict[str, object] = {
        "requirements": entry_json.get("requirements", []),
        "AcademicRequirementsMetaData": metadata,
        "scholarshipName": scholarship_json.get("scholarshipName", ""),
        "scholarshipAmount": scholarship_json.get("scholarshipAmount", ""),
        "scholarshipType": scholarship_json.get("scholarshipType", ""),
        "scholarshipMetaData": normalize_metadata_array(
            scholarship_json.get("scholarshipMetaData"),
            default_subtitle="Scholarships",
        ),
    }
    for key in ENGLISH_TEST_KEYS:
...
def build_output_json(
    ...
    *,
    ...
    output: dict[str, object] = {
        ...
        "courseName": course_name or str(stage1_json.get("courseName", "") or "").strip(),
        "courseUrl": course_url
        or str(stage1_json.get("courseUrl", "") or stage1_json.get("courseUrlExternal", "") or "").strip(),
        "intakeInfo": str(stage1_json.get("intakeInfo", "") or "").strip(),
        "courseDuration": str(stage1_json.get("courseDuration", "") or "").strip(),
        "tuitionFee": str(stage1_json.get("tuitionFee", "") or "").strip(),
        "currency": str(stage1_json.get("currency", "") or "").strip(),
        "initialDeposit": str(stage1_json.get("initialDeposit", "") or "").strip(),
        "applicationFee": str(stage1_json.get("applicationFee", "") or "").strip(),
        "feesMetaData": merge_fees_metadata(
            normalize_metadata_array(stage1_json.get("feesMetaData")),
            normalize_metadata_array(
                llm_json.get("feesMetaData"),
                default_subtitle="Initial tuition Deposit",
            ),
        ),
        "applicationDeadline": str(stage1_json.get("applicationDeadline", "") or "").strip(),
        "requirements": requirements
        if requirements is not None
        else normalize_requirements_list(llm_json.get("requirements")),
        "AcademicRequirementsMetaData": filter_academic_metadata(
            normalize_metadata_array(
                academic_metadata
                if academic_metadata is not None
                else llm_json.get("AcademicRequirementsMetaData")
            )
        ),
        "scholarshipName": str(llm_json.get("scholarshipName", "") or "").strip(),
        "scholarshipAmount": normalize_fee_numeric(str(llm_json.get("scholarshipAmount", "") or "")),
        "scholarshipType": str(llm_json.get("scholarshipType", "") or "").strip(),
        "scholarshipMetaData": normalize_metadata_array(
            llm_json.get("scholarshipMetaData"),
            default_subtitle="Scholarships",
        ),
    }
    for key in ENGLISH_TEST_KEYS:
        # Prefer Stage 2 English scalars; fall back to Stage 1 for IELTS
    ...
    if tuition_fee and currency:
        output["tuitionFeeCandidates"] = [
            {"label": "INTERNATIONAL", "amount": tuition_fee, "currency": currency},
        ]
    return output
...
def run_stage2_llm_part(
    *,
    ...
    save_audit(audit_dir, f"{name}_prompt.txt", prompt)
    print(f"  Stage 2 ({name})", flush=True)
    t0 = time.time()
    parsed, raw = chat(prompt, model=model, host=host)
    print(f"    done in {int(time.time() - t0)}s", flush=True)
    save_audit(audit_dir, f"{name}_response.json", json.dumps(raw, indent=2))
    save_audit(audit_dir, f"{name}_parsed.json", json.dumps(parsed, indent=2, ensure_ascii=False))
    return parsed
...
def merge_stage2_row(
    ...
    *,
    ...
    # Identity always from Python — never trust LLM uniName if present
    ...
    # English test scalars from Stage 2b — fill gaps; PTE/TOEFL prefer English page
    for key in ENGLISH_TEST_KEYS:
        value = str(llm_json.get(key, "") or "").strip()
        if not value:
            continue
        if key.startswith("ielts"):
            ...
            # PTE / TOEFL: English uni page is authoritative when present
            merged[key] = value
    ...
    deposit_fees = normalize_metadata_array(
        llm_json.get("feesMetaData"),
        default_subtitle="Initial tuition Deposit",
    )
    if deposit_fees:
    ...
    if (entry_content or uni_content) and course_level:
        ...
        use_fallback = bool(fallback) and (
            ...
            or (
                ...
                and all(item["degree"] in UG_ENTRY_DEGREES for item in existing)
            )
        )
        if use_fallback:
...
def normalize_row(row: dict, university_name: str, course_url: str) -> dict:
    ...
    if requirements in ("", None) and any(
        row.get(key) for key in ("minDegreeName", "minGpa", "higherDegreeName", "higherGpa")
    ):
        legacy: list[dict[str, str]] = []
        if row.get("minDegreeName") or row.get("minGpa"):
            legacy.append(
                {
                    "degree": str(row.get("minDegreeName", "") or ""),
                    "grade": str(row.get("minGpa", "") or ""),
                }
            )
        if row.get("higherDegreeName") or row.get("higherGpa"):
            legacy.append(
                {
                    "degree": str(row.get("higherDegreeName", "") or ""),
                    "grade": str(row.get("higherGpa", "") or ""),
                }
            )
        requirements = legacy
...
def extract_course(
    ...
    *,
    ...
    prompt_1_template = load_template(PROMPT_1)
    prompt_2_entry = load_template(PROMPT_2_ENTRY)
    prompt_2_english = load_template(PROMPT_2_ENGLISH)
    prompt_2_scholarship = load_template(PROMPT_2_SCHOLARSHIP)
    prompt_2_initial_deposit = load_template(PROMPT_2_INITIAL_DEPOSIT)
    ...
    if skip_stage1 and stage1_path.exists():
        ...
        stage1_content = response_content_from_file(stage1_response_path)
    else:
        stage1_prompt = fill_template(
            prompt_1_template,
            COURSE_NAME=course_name,
            COURSE_URL=course_url,
            INPUT_CONTENT=course_body.strip(),
            KNOWN_FIELDS=json.dumps(parser_hints, indent=2, ensure_ascii=False),
        )
        save_audit(audit_dir, "stage1_prompt.txt", stage1_prompt)
        print(f"  Stage 1: {course_name}", flush=True)
        t0 = time.time()
        stage1_json, stage1_raw = chat(stage1_prompt, model=model, host=host)
        print(f"    done in {int(time.time() - t0)}s", flush=True)
        stage1_content = extract_response_content(stage1_raw)
        save_audit(audit_dir, "stage1_response.json", json.dumps(stage1_raw, indent=2))
    ...
    save_audit(audit_dir, "stage1_parsed.json", json.dumps(stage1_json, indent=2))
    ...
    english_json = run_stage2_llm_part(
        audit_dir=audit_dir,
    ...
    english_json = enrich_english_parsed(
        english_json,
        uni_sections.get("english", ""),
        course_name=course_name,
        course_level=course_level,
        stage1_json=stage1_json,
        course_body=course_body,
    ...
    scholarship_json = run_stage2_llm_part(
        audit_dir=audit_dir,
        name="scholarship",
        prompt=fill_template(
            prompt_2_scholarship,
            COURSE_NAME=course_name,
            COURSE_URL=course_url,
            STAGE1_JSON=stage1_json_text,
            SCHOLARSHIP_CONTENT=uni_sections.get("scholarship", ""),
        ),
        model=model,
        host=host,
    )
    scholarship_json = enrich_scholarship_parsed(
        scholarship_json,
        uni_sections.get("scholarship", ""),
        course_name=course_name,
        course_level=course_level,
        course_body=course_body,
    )
    save_audit(
        audit_dir,
        "scholarship_parsed.json",
        json.dumps(scholarship_json, indent=2, ensure_ascii=False),
    )
    tuition_fee = str(stage1_json.get("tuitionFee", "") or "").strip()
    deposit_json = run_stage2_llm_part(
        audit_dir=audit_dir,
        name="initial_deposit",
        prompt=fill_template(
            prompt_2_initial_deposit,
            COURSE_NAME=course_name,
            COURSE_URL=course_url,
            TUITION_FEE=tuition_fee,
            STAGE1_JSON=stage1_json_text,
            COURSE_CONTENT=course_body.strip(),
            DEPOSIT_CONTENT=uni_sections.get("deposit", ""),
        ),
        model=model,
        host=host,
    )
    deposit_json = enrich_deposit_parsed(
        deposit_json,
        course_content=course_body.strip(),
        deposit_content=uni_sections.get("deposit", ""),
    )
    save_audit(
        audit_dir,
        "initial_deposit_parsed.json",
        json.dumps(deposit_json, indent=2, ensure_ascii=False),
    )
    ...
    # Final identity lock — uniName / URL never come from LLM
...
def run_extraction(
    ...
    *,
    ...
    if presetup:
        ...
        if not sample_urls:
            raise ValueError(
                f"No {output_dir / 'presetup_sample.json'}. Run --presetup first."
            )
        courses = entries_from_presetup_clean(output_dir)
        if not courses:
            raise ValueError(
                "No markdown in output/clean/pre_setup_course matches "
                "presetup_sample.json. Run --presetup first."
            )
    else:
        ensure_course_index_synced(code_dir)
        index_rows = read_course_index_csv(output_dir)
        courses = [index_row_to_entry(row) for row in index_rows]
    ...
    progress = load_progress(output_dir, presetup=presetup)
    ...
    if completed != set(progress.get("completed", [])) or failed != progress.get("failed", []):
        ...
        save_progress(output_dir, progress, presetup=presetup)
    ...
    if md_file:
        ...
        courses = [
            entry
            for entry in courses
            if entry["md_file"] == md_name or Path(entry["md_file"]).name == md_name
        ]
        if not courses:
        ...
        courses = [
            entry for entry in courses if normalize_url(entry.get("course_url") or "") in wanted
        ]
        if not courses:
        ...
        courses = [
            entry
            for entry in courses
            if (entry.get("study_level") or "").strip() in allowed
        ]
        if not courses:
            raise ValueError(
                f"No indexed courses for study level(s): {', '.join(sorted(allowed))}"
            )
    ...
    if presetup:
        output_csv = (
            output_dir / "extracted" / PRESETUP_EXTRACT_SUBDIR / "extracted_courses.csv"
        )
        index_csv = None
        input_label = f"clean/{PRESETUP_CLEAN_SUBDIR}/"
        output_label = f"extracted/{PRESETUP_EXTRACT_SUBDIR}/"
    else:
        output_csv = extracted_csv_path(output_dir)
        index_csv = course_index_path(output_dir)
    ...
    for index, entry in enumerate(courses, start=1):
        ...
        try:
            row, stage1_output, stage2_output, output_json = extract_course(
                code_dir,
                entry,
                model=model,
                host=host,
            ...
            if index_csv is not None:
                print(
                    f"  -> updated {index_csv.name} (stage1_output, stage2_output, output_json)",
                    flush=True,
                )
...
def main(code_dir: Path | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Extract Course.csv rows using Ollama (Stage 1 + Stage 2)."
    )
    parser.add_argument(
        "university_dir",
        nargs="?",
        default=DEFAULT_UNIVERSITY,
        help="University code/ folder (default: current working directory)",
    )
    parser.add_argument("--course-index", type=int, help="Process one course by index (1-based)")
    parser.add_argument(
        "--md-file",
        help="Process one course by md filename (e.g. courses-accounting-and-finance-bsc-hons-2026-27.md)",
    )
    parser.add_argument(
        "--build-index",
        action="store_true",
        help=f"Build {COURSE_INDEX_REL} from clean/courses/*.md",
    )
    ...
    parser.add_argument(
        "--skip-stage1",
        action="store_true",
        help="Reuse existing extracted/{slug}/stage1_parsed.json",
    )
    parser.add_argument(
        "--skip-uni-validation",
        action="store_true",
        help="Skip output/clean/uni validation gate before LLM extraction",
    )
    parser.add_argument(
        "--study-level",
        action="append",
        default=[],
        metavar="LEVEL",
        help="Restrict to a study level (repeatable)",
    )
    parser.add_argument(
        "--url",
        action="append",
        default=[],
        metavar="URL",
        help="Process only this course URL (repeatable)",
    )
    parser.add_argument(
        "--presetup",
        action="store_true",
        help="Extract presetup_sample.json courses from clean/pre_setup_course into extracted/pre_setup_course_extracted",
    )
    args = parser.parse_args()

    try:
        code_dir = configure_code_dir(
            Path(code_dir) if code_dir is not None else Path(args.university_dir)
        )
        if args.build_index:
            build_course_index(code_dir)
            return 0
        run_extraction(
            code_dir,
            ...
            study_levels=args.study_level or None,
