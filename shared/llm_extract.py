...
class ExtractionPathConfig:
    ...
    def is_boilerplate_heading(title: str) -> bool:
        """True for cookie banners and other non-course H1 text left in cleaned markdown."""
        normalized = ' '.join((title or '').split()).casefold()
        if not normalized:
            return True
        if normalized in {'skip to content', 'skip to main content'}:
            return True
        if 'cookie' in normalized and any(
            token in normalized
            for token in ('choice', 'consent', 'preference', 'regarding', 'this site', 'we use')
        ):
            return True
        return False
    ...
    def infer_course_name(body: str, course_url: str) -> str:
        for line in body.splitlines():
            ...
            if stripped.startswith('# '):
                ...
                if not ExtractionPathConfig.is_boilerplate_heading(title):
                    return title
...
is_boilerplate_heading = ExtractionPathConfig.is_boilerplate_heading
