...
class UniCleanValidator:
    """Validate output/clean/uni markdown against pipeline parsers."""

    LEVEL_DEGREE_ALLOWLIST = LEVEL_DEGREE_ALLOWLIST
    ...
    @classmethod
    def _allowed_degrees_for_study_level(cls, study_level: str) -> set[str]:
        pipeline_level = cls._pipeline_level_for_study_level(study_level)
        if not pipeline_level:
            return set()
        return cls.LEVEL_DEGREE_ALLOWLIST[pipeline_level]

    @classmethod
    def validate_bangladesh_entry(cls, content: str, filename: str, report: ValidationReport) -> None:
        ...
        for level_index, level in enumerate(study_levels):
            ...
            allowed_degrees = cls._allowed_degrees_for_study_level(study_level)
        ...
        for pipeline_level in cls.PIPELINE_LEVELS:
    ...
    @classmethod
    def validate_scholarships(cls, content: str, filename: str, report: ValidationReport) -> None:
    ...
    def validate_uni_clean(self, output_dir: Path, *, university_name: str = "") -> ValidationReport:
        ...
        validators = {
            "entry": self.validate_bangladesh_entry,
            "english": self.validate_english_requirements,
            "scholarship": self.validate_scholarships,
...
class UniCleanValidateCLI:
    """Command-line interface for uni clean validation."""

    def __init__(self, validator: UniCleanValidator | None = None) -> None:
        self.validator = validator or UniCleanValidator()
...
_default_validator = UniCleanValidator()
_default_cli = UniCleanValidateCLI(_default_validator)

# Backward-compatible module-level aliases
validate_bangladesh_entry = UniCleanValidator.validate_bangladesh_entry
validate_english_requirements = UniCleanValidator.validate_english_requirements
validate_scholarships = UniCleanValidator.validate_scholarships
validate_deposit = UniCleanValidator.validate_deposit
validate_uni_clean = _default_validator.validate_uni_clean
format_report = UniCleanValidator.format_report
ensure_uni_clean_valid = _default_validator.ensure_uni_clean_valid
