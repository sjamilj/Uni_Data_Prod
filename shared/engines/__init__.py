...
class EngineRegistry:
    """Resolve built-in or plugin course HTML clean engines."""
    ...
    def get(cls, engine_name: str, code_dir: Path):
        ...
        if name == "auto":
            ...
            if module is not None:
                return cls.get("plugin", code_dir)
            return GenericCourseHtmlEngine

        try:
            return cls.BUILTIN_ENGINES[name]
        except KeyError as exc:
            known = ", ".join(sorted(cls.BUILTIN_ENGINES))
            raise ValueError(
...
_BUILTIN_ENGINES = EngineRegistry.BUILTIN_ENGINES
_load_plugin_module = EngineRegistry._load_plugin_module
get_course_html_engine = EngineRegistry.get
